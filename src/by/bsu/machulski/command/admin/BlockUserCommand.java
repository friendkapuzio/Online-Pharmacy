...
public class BlockUserCommand extends AbstractCommand {
    ...
    public Router execute(SessionRequestContent content) {
        ...
        try {
            ...
            boolean isDeleted = userLogic.block(userId);
            String path = ConfigurationManager.getPath(PageConfigConstant.MESSAGE);
            if (isDeleted) {
                path = ConfigurationManager.addParameter(path, MESSAGE_PARAMETER, AdminActionMessage.USER_BLOCK_SUCCEED);
            } else {
                path = ConfigurationManager.addParameter(path, MESSAGE_PARAMETER, AdminActionMessage.USER_BLOCK_FAILED);
            }
