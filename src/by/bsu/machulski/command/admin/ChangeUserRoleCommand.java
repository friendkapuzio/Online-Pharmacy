...
public class ChangeUserRoleCommand extends AbstractCommand {
    ...
    public Router execute(SessionRequestContent content) {
        ...
        try {
            ...
            String newRole = content.getFirstParameterValue(UserAttributeConstant.NEW_ROLE);
            UserLogic userLogic = new UserLogic();
            boolean isChanged = userLogic.changeRole(userId, newRole);
            String path = ConfigurationManager.getPath(PageConfigConstant.MESSAGE);
            if (isChanged) {
                path = ConfigurationManager.addParameter(path, MESSAGE_PARAMETER, AdminActionMessage.ROLE_CHANGE_SUCCEED);
            } else {
                path = ConfigurationManager.addParameter(path, MESSAGE_PARAMETER, AdminActionMessage.ROLE_CHANGE_FAILED);
            }
